...
sub gate($$$;$)                                                                 # A L<lg> of some sort to be added to the L<chip>.
 {my ($chip, $type, $output, $inputs) = @_;                                     # Chip, gate type, output name, input names to output from another gate
  ...
  if ($type =~ m(\A(input)\Z)i)                                                 # Input gates input to themselves unless they have been connected to an output gate during sub chip expansion
   {defined($inputs) and confess <<"END";
...
END
   ...
   {if (defined($inputs))
     {ref($inputs) and confess <<"END";
Scalar input name required for output gate: '$output'
END
   ...
   {!defined($inputs) and confess "Input name required for gate: '$output'\n";
    $type =~ m(\Anot\Z)i and ref($inputs) =~ m(hash)i and confess <<"END";
...
END
   ...
   {!defined($inputs) and confess <<"END" =~ s/\n/ /gsr;
Input hash required for gate: '$output'
END
    ref($inputs) =~ m(hash)i or confess <<"END" =~ s/\n/ /gsr;
Inputs must be a hash of input names to outputs for gate: '$output'
to show the output accepted by each input. Input gates have no inputs,
they are supplied instead during simulation\n";
END
    keys(%$inputs) == 2 or confess <<"END" =~ s/\n/ /gsr;
Two inputs required for gate: '$output'
END
   }
...
END
    ref($inputs) =~ m(hash)i or confess <<"END" =~ s/\n/ /gsr;
Inputs must be a hash of input gate
...
during simulation
END
#   keys(%$inputs) < 2 and confess <<"END" =~ s/\n/ /gsr;
...
No output driving input '$o' on gate '$G'
END
...
sub compareGt($$$$$%)                                                           # Compare two unsigned binary integers of specified width and return B<1> if the first integer is more than B<b> else B<0>.
 {my ($chip, $output, $a, $b, $bits, %options) = @_;                            # Chip, name of component also the output bus, first integer, second integer, options
  ...
  $chip->nxor (n("$o.e", $_), {1=>n($a, $_), 2=>n($b, $_)}) for 2..$B;          # Test all but the lowest bit pair for equality
 ...
 {my ($chip, $output, $input, $bits, %options) = @_;                            # Chip, output name, input mask, number of bits in mask, options
  ...
  for my $b(1..$B)                                                              # Each bit of the mask
   {my @s = (reverse split //, sprintf "%0${bits}b", $b);                       # Bits for this point in the mask
    ...
    $chip-> or(n($o, $b), {map {($_ => n("$o.a", $_))} 1..$b});                 # Set mask
   }
...
Create a chip to compare two 4 bit integers for equality:
...
36 L<Silicon::Chip::Simulation::bitsToInteger|/Silicon::Chip::Simulation::bitsToInteger> - Represent the state of bits in the simulation results as an unsigned binary integer.

37 L<Silicon::Chip::Simulation::print|/Silicon::Chip::Simulation::print> - Print simulation results as text.

38 L<Silicon::Chip::Simulation::printSvg|/Silicon::Chip::Simulation::printSvg> - Print simulation results as svg.

39 L<Silicon::Chip::Simulation::wordsToInteger|/Silicon::Chip::Simulation::wordsToInteger> - Represent the state of words in the simulation results as an array of unsigned binary integer.

40 L<Silicon::Chip::Simulation::wordXToInteger|/Silicon::Chip::Simulation::wordXToInteger> - Represent the state of words in the simulation results as an array of unsigned binary integer.

41 L<simulate|/simulate> - Simulate the action of the L<logic gates|https://en.wikipedia.org/wiki/Logic_gate> on a L<chip|https://en.wikipedia.org/wiki/Integrated_circuit> for a given set of inputs until the output value of each L<logic gate|https://en.wikipedia.org/wiki/Logic_gate> stabilizes.
...
#latest:;
if (1)                                                                          #TnewChip #TSilicon::Chip::Simulation::printSvg #TSilicon::Chip::Simulation::print # Single AND gate
 {my $c = Silicon::Chip::newChip;
  $c->input ("i1");
  $c->input ("i2");
  $c->and   ("and1", {1=>q(i1), 2=>q(i2)});
